...
const priorities = [
  { value: "critical", label: "严重", icon: AlertTriangle, color: priorityColor.critical ?? priorityColorDefault },
  ...
  const createIssue = useMutation({
    ...
    onSuccess: ({ issue, companyId, failures }) => {
      ...
      if (failures.length > 0) {
        ...
        pushToast({
          title: `已创建 ${issueRef}，但上传存在警告`,
          body: `${failures.length} 个暂存文件未能添加。`,
          tone: "warn",
          action: prefix
          ...
          {currentProject && currentProjectSupportsExecutionWorkspace && (
            ...
            <div className="space-y-1.5">
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground">
                  正在复用 {selectedReusableExecutionWorkspace.name}，来源为 {selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? "现有执行工作区"}。
                </div>
              )}
              ...
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  ...
                  {assigneeModelLane === "primary" && (
                    <p className="text-[11px] text-muted-foreground">在代理主模型上运行。</p>
                  )}
        ...
        {assigneeValue && status === "backlog" ? (
          ...
          >
            ...
            <span className="leading-snug">
              分配负责人表示任务可执行；只有想刻意暂存时才保持为 <span className="font-medium">待办池</span>。状态变为 <span className="font-medium">待办</span> 或 <span className="font-medium">进行中</span> 前，不会唤醒负责人。
            </span>
          </div>
        ) : null}
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                ...
                <span>{createIssue.isPending ? "正在创建..." : isSubIssueMode ? "创建子任务" : "创建任务"}</span>
              </span>
            </Button>
          </div>
        </div>
